  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        <Link href="/">
          <span className="flex items-center gap-2 text-lg font-semibold tracking-tight text-gray-900 dark:text-gray-100">
        ...
        <nav className="hidden md:flex items-center gap-1">
          {toolLinks.map((link) => (
            <Link key={link.href} href={link.href}>
              <a className="px-3 py-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                {link.label}
              </a>
            </Link>
          ))}
          <Link href="/blog">
            <a className="px-3 py-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Blog</a>
          </Link>
        </nav>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
          <nav className="max-w-6xl mx-auto px-4 py-3 flex flex-col gap-1">
            {toolLinks.map((link) => (
              <Link key={link.href} href={link.href}>
                <a className="py-2 px-3 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-md" onClick={() => setMobileMenuOpen(false)}>
                  {link.label}
                </a>
              </Link>
            ))}
            ...
            <Link href="/blog">
              <a className="py-2 px-3 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-md" onClick={() => setMobileMenuOpen(false)}>Blog</a>
            </Link>
          </nav>
        </div>
      )}
